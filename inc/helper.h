...
inline void ReadAndPreprocess(const size_t max_points,
                              const unsigned char *const src_ptr,
                              const size_t src_bytes,
                              const size_t src_point_bytes,
                              std::vector<float> *dest) {
    static constexpr auto dest_point_feats = 4;
    static constexpr float x_mim = -40, x_max = 40, y_min = -40, y_max = 40, r2_min = 0;

    const auto src_point_num = src_bytes / src_point_bytes;
    dest->resize((max_points > src_point_num ? max_points : src_point_num) * dest_point_feats, 0.f);
    ...
    for (; src_cur <= src_end; src_cur += src_point_bytes) {
        ...
        auto x = p[0], y = p[1], z = p[2], intensity = 0.f;
        auto r2 = x * x + y * y + z * z;
