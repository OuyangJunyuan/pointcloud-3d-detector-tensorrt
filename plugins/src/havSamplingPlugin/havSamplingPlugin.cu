    ...
    nvinfer1::DynamicPluginTensorDesc const* out, int32_t nbOutputs) noexcept
{
    try
    {
        ...
        mTableSize = get_table_size(mNbPoints, 2048);
    }
    catch (std::exception const& e)
    {
        caughtError(e);
    ...
    nvinfer1::PluginTensorDesc const* outputs, int32_t nbOutputs) const noexcept
{
    try
    {
        ...
        size_t batchMaskSize = batchSize * sizeof(bool);
        ...
        //        printf("%zx\n", calculateTotalWorkspaceSize(workspaces, 7));
    ...
    cudaStream_t stream) noexcept
{
    try
    {
        ...
        bool* batchMaskPtr = static_cast<bool*>(workspace);
        auto* hashTablePtr
            = reinterpret_cast<uint32_t*>(nextWorkspacePtr(reinterpret_cast<int8_t*>(batchMaskPtr), batchMaskSize));
        auto* distTablePtr
            = reinterpret_cast<float*>(nextWorkspacePtr(reinterpret_cast<int8_t*>(hashTablePtr), hashTableSize));
        auto* pointSlotPtr
            = reinterpret_cast<uint32_t*>(nextWorkspacePtr(reinterpret_cast<int8_t*>(distTablePtr), distTableSize));
        auto* pointDistPtr
            = reinterpret_cast<float*>(nextWorkspacePtr(reinterpret_cast<int8_t*>(pointSlotPtr), pointSlotSize));
        auto* sampledNumPtr
            = reinterpret_cast<uint32_t*>(nextWorkspacePtr(reinterpret_cast<int8_t*>(pointDistPtr), pointDistSize));
        auto* voxelPtr
        ...
        thrust::device_ptr<bool> dev_batch_mask_ptr(batchMaskPtr);
...
//            print((uint32_t*) hashTablePtr, {batchSize, (int)mTableSize}, "HAVSampling::hashTablePtr");
        ...
        unique_mini_dist_kernel<<<blocks, threads, 0, stream>>>(batchSize, mNbPoints, mTableSize, max, pointCloudPtr,
            voxelPtr, hashTablePtr, distTablePtr, pointSlotPtr, pointDistPtr);

        set_mask_kernel<<<blocks, threads, 0, stream>>>(batchSize, mNbPoints, mNbSamples, mTableSize, pointSlotPtr,
            pointDistPtr, distTablePtr, static_cast<uint32_t*>(sampledIndPtr), sampledNumPtr);
#ifdef DEBUG
        cudaStreamSynchronize(stream);
